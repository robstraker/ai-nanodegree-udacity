...
class SelectorBIC(ModelSelector):
    ...
    def select(self):
        ...
        best_score, best_model, logL = float("inf"), None, None
        ...
        for n in range(self.min_n_components, self.max_n_components + 1):
            try:
                ...
                n_param = (n**2) + (2 * n_features * n) - 1
                bic = (-2 * logL) + (n_param * logN)
                if bic < best_score:
...
class SelectorDIC(ModelSelector):
    ...
    def select(self):
        ...
        # DIC = log(L(original word)) - average(log(L(otherwords)))
        
        best_score, best_model, logL = float("-inf"), None, None 
        
        for n in range(self.min_n_components, self.max_n_components + 1):
            other_word_penalty = 0.0

            try:
                model = self.base_model(n)
                logL = model.score(self.X, self.lengths)
                other_word_penalty = np.mean([model.score(self.hword[word]) for word in self.words if word != self.this_word])
                DIC = logL - other_word_penalty
                if DIC > best_score:
        ...
        for n in range(self.min_n_components, self.max_n_components + 1):
            ...
            try:
            	for train_idx, test_idx in split_method.split(self.sequences):
                	...
                	model = GaussianHMM(n_components=n, covariance_type="diag", n_iter=1000, random_state=self.random_state, verbose=False).fit(X_train, lengths_train)
