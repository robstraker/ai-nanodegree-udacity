...
detector.addEventListener("onImageResultsSuccess", function(faces, image, timestamp) {
  ...
  if (faces.length > 0) {
    ...
    // Give player 10 seconds to mimic each target emoji
    displayText(canvas, 'Can you mimic each emoji in < 10 seconds?', 350);

	if (gameFlag) {
    	if (playGame(faces[0], targetCode)) {
    		correct += 1;
    		setScore(correct, total);
    		targetCode = selectEmoji();
    	}
    }
  }
});
...
// Select next random emoji from list recognized by Affectiva 
function selectEmoji() {
  var randomCode = Math.floor(Math.random() * (emojis.length-1));
  var nextEmoji = emojis[randomCode];
  setScore(correct, total);
  
// Limit game to 10 target emojis
  if (total == 10) {
  ...
  total += 1;
  setTargetEmoji(nextEmoji);

  // Set timeout for each target emoji
  setTimeout(function() {
    if (targetCode == nextEmoji && gameFlag) {
      console.log('Timeout on this emoji!');
      targetCode = selectEmoji();
    } else if (gameFlag) {
...
// Draw message on display
function displayText(canvas, text, pos) {
    var ctx = canvas.getContext('2d');
    ctx.fillStyle = 'white';
...
// Determine whether there is a match to target emoji
function playGame(face, targetCode) {
  dominantCode = toUnicode(face.emojis.dominantEmoji);
  if (dominantCode == targetCode) {
